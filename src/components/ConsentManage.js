...
const ConsentManage = ({ role, user }) => {
  ...
  useEffect(() => {
    const fetchConsents = async () => {
      const q = query(collection(db, "consent"), where("userEmail", "==", user.email));
      const querySnapshot = await getDocs(q);
      setConsents(querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    };
    fetchConsents();
  }, [user]);
    ...
    <Container>
      ...
      <List>
        {consents.map(({ id, data, operationsPermitted, status }) => (
          <ListItem key={id}>
            <ListItemText primary={`Data: ${data}, Operations Permitted: ${operationsPermitted}, Status: ${status}`} />
            <Button onClick={() => updateConsent(id, "accepted")} color="success">Accept</Button>
            <Button onClick={() => updateConsent(id, "rejected")} color="error">Reject</Button>
          </ListItem>
        ))}
      </List>
    </Container>
